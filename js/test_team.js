const teamworkTest = window.CAMPUS_PSYCH_TESTS.teamwork;
const questions = teamworkTest.questions;
...
function showResult() {
    ...
    const resultText = teamworkTest.results[highest].text;

    document.getElementById("quiz-box").innerHTML = `
    <h2>당신의 유형은?</h2>
    <p id='result'>${resultText}</p>
    <div class="mt-4 text-center">
        <a href="${teamworkTest.listUrl}" class="btn btn-outline-primary me-2">다른 테스트 하러 가기</a>
        <a href="${teamworkTest.guideUrl}" class="btn btn-primary">유형별 해설 보기</a>
    </div>
`;
}
